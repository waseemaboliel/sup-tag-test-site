...
- An `api-errors.html` page:
  - Buttons firing real `fetch` calls to generate genuine API errors under the default ≥400 rule. `httpstat.us` turned out to be unreliable in testing (intermittent 503s / CORS failures) — using `jsonplaceholder.typicode.com/posts/999999` (verified stable 404) and `reqres.in/api/login` with an empty POST body (verified stable 400: "Missing email or username") instead.
  - A button firing a request to a URL with PII in the query string (e.g. `jsonplaceholder.typicode.com/posts/999999?email=test@test.com`), plus buttons demonstrating the masking commands `window._uxa.push(['networkRequest:maskUrls', ...])` (partial match) and `['api-errors:maskUrl', ...])` (full match) so we can confirm masking actually strips the PII before it's collected — directly relevant to SUP-21751-style "masking rule not applying" tickets.
